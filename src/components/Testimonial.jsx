    ...
    <Swiper 
     ...
     {testimonialsItems.map((item) => (
      <SwiperSlide key={item.id} className="bg-white border border-white-95 p-8 lg:p-10 rounded-[10px] space-y-3">
       <p>{item.text}</p>
       <div className="flex items-center justify-center flex-wrap pt-4 border-t border-t-orange-90 gap-4">
        <div className="flex items-center gap-3">
         <div className="">
          <img src={`assets/${item.img}`} alt={item.author} width={50} height={50} />
         </div>
